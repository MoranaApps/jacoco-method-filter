...
object JacocoFilteredSettings_backup_before_jacoco_agent extends AutoPlugin {
    ...
    Seq(
      ...
      jacocoFiltered := {
        ...
        try {
          ...
          // Force sbt-jacoco to re-instrument from the (now filtered) compiledDir
          val instrDir = (Test / crossTarget).value / "jacoco" / "instrumented-classes"
          if (instrDir.exists()) {
            log.info(s"[jmf] deleting old instrumented-classes: ${instrDir.getAbsolutePath}")
            IO.delete(instrDir)
          }
          // also ensure the exec from a previous run is gone
          ...
          val instrCount = (instrDir ** GlobFilter("*.class")).get.size
          log.info(s"[jmf] jacoco instrumented -> $instrDir (count=$instrCount)")
        } finally {
        ...
        // STEP 7: wait until exec exists AND is stable (avoids FS race)
        // helper: wait until file exists and size is stable twice in a row
        def waitForExec(f: File, timeoutMs: Int = 10000, pollMs: Int = 200): File = {
          val deadlineNs = System.nanoTime() + timeoutMs.toLong * 1000000L
          var lastLen    = -1L
          var stableHits = 0
          while (System.nanoTime() < deadlineNs) {
            if (f.exists()) {
              ...
              if (len > 0 && len == lastLen) {
                stableHits += 1
                if (stableHits >= 2) return f   // seen same non-zero size twice → likely fully written
              } else {
                stableHits = 0
                lastLen = len
              }
        ...
        log.info(s"[jmf] waiting for exec at: ${expectedExec.getAbsolutePath}")
        val exec     = waitForExec(expectedExec)
